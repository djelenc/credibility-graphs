...
* When the number of agents increases (even to 20), the performance decreases beyond reason. The issue is g merging: as the number of agents increases, so does the number of opinions, and consequently the number of loops that are created during g merge. Enumerating all of them simply takes too long.
